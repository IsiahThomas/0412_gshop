<template>
  <div class="shop-header">
    ...
    <div class="shop-header-discounts" @click="isShowDown=true">
      <div class="discounts-left">
        <div class="activity activity-green">

          <span class="content-tag" v-if="info.supports">
            <!--为什么要用v-if判断一下呢？
            cannot read property '0' of undefined
            初始显示出现问题，解决方法，加一个v-if="false",当它为false时，
            初始化的时候模板都不会解析，-->
      ...
      <div class="discounts-right" v-if="info.supports">
        {{info.supports.length}}个优惠
      </div>
    </div>
    <div class="shop-brief-modal" :style="isShowDetail ? 'display: block':'display: none'">
      <div class="brief-modal-content">
        ...
        <div class="mask-footer">
          <span class="iconfont icon-close" @click="isShowDetail=false"></span>
        </div>
      </div>
      <div class="brief-modal-cover"></div>
    </div>
    <div class="activity-sheet" :style="isShowDown ? 'display: block':'display:none;'">
      <div class="activity-sheet-content">
        <h2 class="activity-sheet-title">
          优惠活动</h2>
        <ul class="list">
          <li class="activity-item" v-for="(p,index) in info.supports" :key="index"
          :class="{'activity-green':index === 0,'activity-red':index === 1,'activity-orange':index!==0&&index!==1}">
            <span class="content-tag">
              <span class="mini-tag">{{p.name}}</span>
            </span>
            <span class="activity-content">{{p.content}}</span>
          </li>
        </ul>
        <div class="activity-sheet-close" @click="isShowDown=false">
